    ...
    <%--    </div>--%>
                                ...
                                <h3>
                                    <c:choose>
                                        ...
                                        <c:when test="${order.status == 5}">
                                            <span class="status-tag" style="background-color: #dc3545;">配送员放弃，待审核</span>
                                        </c:when>
                                        <c:otherwise>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="info-card">
                    <h4>申请详情</h4>
                    <p><strong>申请理由：</strong>${order.abandonReason}</p>
                    <p><strong>详细说明：</strong></p>
                    <textarea class="form-control" rows="5" readonly
                              style="flex: 1; text-align: left; resize: none; padding: 8px;">
                        <c:out value="${order.abandonDescription != null ? order.abandonDescription : ''}"/>
                    </textarea>
...
<%--                                <button type="button" class="btn btn-danger btn-sm">不通过</button>--%>
                                <a href="${pageContext.request.contextPath}/admin/reviewAbandonOrder?orderId=${order.orderId}&result=pass"
                                   class="btn btn-success btn-lg">通过</a>
                                <a href="${pageContext.request.contextPath}/admin/reviewAbandonOrder?orderId=${order.orderId}&result=reject"
                                   class="btn btn-danger btn-lg">不通过</a>
                            </c:when>

<%--                            <!-- 已审核状态 -->--%>
                            <c:otherwise>
                                <span class="badge badge-secondary p-2" style="transform: scale(1.3); display: inline-block;">已审核</span>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
